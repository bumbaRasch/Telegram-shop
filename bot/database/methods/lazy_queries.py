...
from bot.database.models import BoughtGoods, Categories, Goods, ItemValues, Operations, ReferralEarnings, User
...
async def query_categories(offset: int = 0, limit: int = DEFAULT_PAGE_SIZE, count_only: bool = False) -> Any:
    ...
    async with Database().session() as s:
        ...
        result = await s.execute(select(Categories.name).order_by(Categories.name.asc()).offset(offset).limit(limit))
        return [row[0] for row in result.all()]


async def query_items_in_category(
    category_name: str, offset: int = 0, limit: int = DEFAULT_PAGE_SIZE, count_only: bool = False
) -> Any:
    ...
    async with Database().session() as s:
        ...
        result = await s.execute(query.order_by(Goods.name.asc()).offset(offset).limit(limit))
        return [row[0] for row in result.all()]


async def query_user_bought_items(
    user_id: int, offset: int = 0, limit: int = DEFAULT_PAGE_SIZE, count_only: bool = False
) -> Any:
    """Query user's bought items with pagination"""
    async with Database().session() as s:
        if count_only:
            return (
                await s.execute(select(func.count()).select_from(BoughtGoods).where(BoughtGoods.buyer_id == user_id))
            ).scalar() or 0
...
async def query_items_in_position(
    item_name: str, offset: int = 0, limit: int = DEFAULT_PAGE_SIZE, count_only: bool = False
) -> Any:
    ...
    async with Database().session() as s:
        ...
        if count_only:
            count_result = await s.execute(select(func.count()).select_from(query.subquery()))
            return count_result.scalar() or 0
        result = await s.execute(query.order_by(ItemValues.id.asc()).offset(offset).limit(limit))
        return [row[0] for row in result.all()]


async def query_user_referrals(
    user_id: int, offset: int = 0, limit: int = DEFAULT_PAGE_SIZE, count_only: bool = False
) -> Any:
    ...
    async with Database().session() as s:
        if count_only:
            return (
                await s.execute(select(func.count(User.telegram_id)).where(User.referral_id == user_id))
            ).scalar() or 0
...
async def query_referral_earnings_from_user(
    referrer_id: int, referral_id: int, offset: int = 0, limit: int = DEFAULT_PAGE_SIZE, count_only: bool = False
) -> Any:
    ...
    async with Database().session() as s:
        ...
        if count_only:
            ...
            return count_result.scalar() or 0
        result = await s.execute(base.order_by(desc(ReferralEarnings.created_at)).offset(offset).limit(limit))
        return result.scalars().all()


async def query_all_referral_earnings(
    referrer_id: int, offset: int = 0, limit: int = DEFAULT_PAGE_SIZE, count_only: bool = False
) -> Any:
    """Query all referral earnings for user"""
    async with Database().session() as s:
        base = select(ReferralEarnings).where(ReferralEarnings.referrer_id == referrer_id)
        if count_only:
            count_result = await s.execute(select(func.count()).select_from(base.subquery()))
            return count_result.scalar() or 0
        result = await s.execute(base.order_by(desc(ReferralEarnings.created_at)).offset(offset).limit(limit))
...
async def query_promo_codes(offset: int = 0, limit: int = DEFAULT_PAGE_SIZE, count_only: bool = False) -> Any:
    ...
    async with Database().session() as s:
        ...
        result = await s.execute(select(PromoCodes).order_by(desc(PromoCodes.created_at)).offset(offset).limit(limit))
...
async def query_user_operations_history(
    user_id: int, offset: int = 0, limit: int = DEFAULT_PAGE_SIZE, count_only: bool = False
) -> Any:
    ...
    async with Database().session() as s:
        ...
        result = await s.execute(select(combined).order_by(combined.c.date.desc()).offset(offset).limit(limit))
        return [
            {
                "id": row.id,
                "type": row.type,
...
async def query_item_reviews(
    ...
    async with Database().session() as s:
        ...
        result = await s.execute(base.order_by(desc(Reviews.created_at)).offset(offset).limit(limit))
        return [
            {
                "id": r.id,
                "user_id": r.user_id,
