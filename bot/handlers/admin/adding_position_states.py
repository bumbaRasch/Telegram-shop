...
from bot.database.methods import add_values_to_item, check_category_cached, create_item, get_item_info_cached
...
async def check_item_name_for_add(message: Message, state):
    ...
    if item:
        await message.answer(localize("admin.goods.add.name.exists"), reply_markup=back("goods_management"))
        return
...
async def check_category_for_add_item(message: Message, state):
    ...
    if not category:
        await message.answer(localize("admin.goods.add.category.not_found"), reply_markup=back("goods_management"))
        return

    await state.update_data(item_category=category_name)
    await message.answer(
        localize("admin.goods.add.infinity.question"), reply_markup=question_buttons("infinity", "goods_management")
    )
...
async def collect_item_value(message: Message, state):
    ...
    await message.answer(
        ...
        reply_markup=simple_buttons(
            [(localize("btn.add_values_finish"), "finish_adding_items"), (localize("btn.back"), "goods_management")],
            per_row=1,
        ),
    )
...
async def finish_adding_items_callback_handler(call: CallbackQuery, state):
    ...
    text_lines = [localize("admin.goods.add.result.created"), localize("admin.goods.add.result.added", n=added)]
    if skipped_db_dup:
...
async def finish_adding_item_callback_handler(message: Message, state):
    ...
    await log_audit(
        ...
        resource_type="Item",
        resource_id=item_name,
        details=f"admin={admin_info.first_name}, infinite=true",
    )
