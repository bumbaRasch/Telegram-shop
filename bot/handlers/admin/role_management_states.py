...
async def role_management_handler(call: CallbackQuery, state: FSMContext):
    ...
    await edit_msg(call.message, localize("admin.roles.list_title"), reply_markup=simple_buttons(buttons, per_row=1))
...
async def role_view_handler(call: CallbackQuery):
    ...
    text = localize(
        "admin.roles.detail", name=role["name"], perms=_format_permissions(role["permissions"]), users=user_count
    )
...
async def role_create_start(call: CallbackQuery, state: FSMContext):
    await edit_msg(call.message, localize("admin.roles.prompt_name"), reply_markup=back("role_mgmt"))
...
async def role_create_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        localize("admin.roles.select_perms", name=name), reply_markup=_build_perms_keyboard(0, caller_perms)
    )
...
async def role_edit_start(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(
        role_id=role_id, role_name=role["name"], role_perms=role["permissions"], caller_perms=caller_perms, mode="edit"
    )
    await edit_msg(call.message, localize("admin.roles.edit_name_prompt"), reply_markup=back(f"role_v_{role_id}"))
...
async def role_edit_name(message: Message, state: FSMContext):
    ...
    if text == "/skip":
        ...
        if not name or len(name) > 64:
            await message.answer(
                localize("admin.roles.name_invalid"), reply_markup=back(f"role_v_{data.get('role_id')}")
            )
            return
    ...
    await message.answer(
        localize("admin.roles.select_perms", name=name), reply_markup=_build_perms_keyboard(current_perms, caller_perms)
    )
...
async def _toggle_perm(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_reply_markup(reply_markup=_build_perms_keyboard(current, caller_perms))
    await call.answer()
...
async def _perms_done(call: CallbackQuery, state: FSMContext):
    ...
    if mode == "create":
        ...
        if role_id is None:
            await edit_msg(call.message, localize("admin.roles.name_exists"), reply_markup=back("role_mgmt"))
        else:
            await log_audit(
                "create_role",
            ...
            await edit_msg(call.message, localize("admin.roles.created", name=name), reply_markup=back("role_mgmt"))
    elif mode == "edit":
        ...
        if success:
            ...
            await edit_msg(call.message, localize("admin.roles.updated", name=name), reply_markup=back("role_mgmt"))
        else:
            await edit_msg(
                call.message,
...
async def role_delete_confirm(call: CallbackQuery):
    ...
    if success:
        ...
        await edit_msg(call.message, localize("admin.roles.deleted"), reply_markup=back("role_mgmt"))
    else:
        await edit_msg(call.message, localize("admin.roles.delete_fail", error=err), reply_markup=back("role_mgmt"))
...
async def assign_role_confirm(call: CallbackQuery):
    ...
    try:
        await call.message.bot.send_message(
            chat_id=target_id, text=localize("admin.roles.assigned_notify", role=role["name"]), reply_markup=close()
        )
