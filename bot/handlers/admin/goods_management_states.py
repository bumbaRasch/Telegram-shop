...
from bot.keyboards.inline import back, lazy_paginated_keyboard, simple_buttons
...
async def show_str_item(message: Message, state: FSMContext):
    ...
    if total == 0:
        await message.answer(localize("admin.goods.list_in_position.empty"), reply_markup=back("goods_management"))
        await state.clear()
        return
...
async def navigate_items_in_goods(call: CallbackQuery, state: FSMContext):
    ...
    if total == 0:
        await edit_msg(
            call.message, localize("admin.goods.list_in_position.empty"), reply_markup=back("goods_management")
        )
        return
...
async def item_info_callback_handler(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(
        delete_item_id=item_id, delete_item_hash=item_hash, delete_page=page, delete_item_name=item_info["item_name"]
    )
...
async def process_delete_item_from_position(call: CallbackQuery, state: FSMContext):
    ...
    if not item_info:
        ...
        await edit_msg(
            call.message, localize("admin.goods.list_in_position.title"), reply_markup=back(f"gip_{item_hash}_{page}")
        )
        return
    ...
    if item_hash and item_name:
        try:
            ...
            await edit_msg(
                call.message, localize("admin.goods.item.deleted"), reply_markup=back(f"gip_{item_hash}_{page}")
            )
            return
        ...
        if total == 0:
            ...
            await state.update_data(
                items_in_position_paginator=paginator.get_state(), item_hash_mapping={item_hash: item_name}
            )
    else:
        await edit_msg(call.message, localize("admin.goods.item.deleted"), reply_markup=back("goods_management"))

    admin_info = await call.message.bot.get_chat(call.from_user.id)
    await log_audit(
        "delete_item_value",
        user_id=call.from_user.id,
        resource_type="ItemValue",
        resource_id=str(item_id),
        details=f"admin={admin_info.first_name}, position={position_name or '<?>'}",
    )
